...
internal sealed class ActionObserver : ExpressionReconstructor
{
        ...
        Expression[] GetArgumentExpressions(Invocation invocation, Match[] matches)
        {
            ...
            if (matches.Length > 0)
            {
                ...
                for (int argumentIndex = 0; matchIndex < matches.Length && argumentIndex < expressions.Length; ++argumentIndex)
                {
                    ...
                    if (!Equals(invocation.Arguments[argumentIndex], defaultValue))
                    {
    ...
    private static IProxy CreateProxy(Type type, object[]? ctorArgs, MatcherObserver matcherObserver, out Recorder recorder)
    {
        ...
        return (IProxy)ProxyFactory.Instance.CreateProxy(type, recorder, Type.EmptyTypes, ctorArgs ?? Array.Empty<object>());
    }
    ...
    private sealed class Recorder : IInterceptor
    {
        ...
        public IEnumerable<Match> Matches
        {
            get
            {
                Debug.Assert(invocationTimestamp != default);
                return matcherObserver.GetMatchesBetween(creationTimestamp, invocationTimestamp);
            }
        }
        ...
        public void Intercept(Invocation invocation)
        {
            ...
            //if (this.invocation == null)
            {
                ...
                {
                    var result = CreateProxy(awaitableFactory.ResultType, null, matcherObserver, out _);
                    returnValue = awaitableFactory.CreateCompleted(result);
                }
                else if (returnType.IsMockable())
                {
                    returnValue = CreateProxy(returnType, null, matcherObserver, out _);
                }
                else
                {
