...
public abstract partial class Mock : IFluentInterface
{
    ...
    /// <summary>
    /// Gets or sets the <see cref="MockU.DefaultValueProvider"/> instance that will be used
    ...
    public void Verify()
    {
        Verify(setup => setup.IsVerifiable, verifiedMocks: new HashSet<Mock>());
    }
    ...
    {
        Verify(setup => true, verifiedMocks: new HashSet<Mock>());
    }
    ...
    internal void Verify(Func<ISetup, bool> predicate, HashSet<Mock> verifiedMocks)
    {
        ...
        foreach (Invocation invocation in MutableInvocations)
        {
        ...
        foreach (var setup in MutableSetups.FindAll(setup => !setup.IsConditional && predicate(setup)))
        {
            try
            {
    ...
    {
        return InheritedInterfaces.Contains(interfaceType)
            || AdditionalInterfaces.Contains(interfaceType);
    }
    ...
    internal object GetDefaultValue(MethodInfo method, out Mock candidateInnerMock, DefaultValueProvider useAlternateProvider = null)
    {
        ...
        var result = (useAlternateProvider ?? DefaultValueProvider).GetDefaultReturnValue(method, this);
