...
public static class It
{
    ...
    private static readonly MethodInfo isAnyMethod = typeof(It).GetMethod(nameof(IsAny), BindingFlags.Public | BindingFlags.Static);
    ...
    ///     (VB.NET), use <see cref="IsValueType"/> instead.
    ///   </para>
    ///   <para>
    ...
    {
        return Match.Create(actual => comparer.Equals(actual, value), () => Is(value, comparer));
    }
    ...
    {
        return Match.Create(value => items.Contains(value), () => IsIn(items));
    }
    ...
    {
        return Match.Create(value => items.Contains(value, comparer), () => IsIn(items, comparer));
    }
    ...
    {
        return Match.Create(value => !items.Contains(value), () => IsNotIn(items));
    }
    ...
    {
        return Match.Create(value => !items.Contains(value, comparer), () => IsNotIn(items, comparer));
    }
    ...
    public static string IsRegex(string regex)
    {
        ...
        return Match.Create(value => value != null && re.IsMatch(value), () => IsRegex(regex));
    }
    ...
    public static string IsRegex(string regex, RegexOptions options)
    {
        ...
        // But evaluated every time :)
        return Match.Create(value => value != null && re.IsMatch(value), () => IsRegex(regex, options));
    }
