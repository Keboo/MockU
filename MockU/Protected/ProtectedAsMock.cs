    ...
    
{
    ...
    public ISetup<T> Setup(Expression<Action<TAnalog>> expression)
    {
        ...
        var setup = Mock.Setup(mock, rewrittenExpression, null);
    ...
    public ISetupGetter<T, TProperty> SetupGet<TProperty>(Expression<Func<TAnalog, TProperty>> expression)
    {
        ...
        var setup = Mock.SetupGet(mock, rewrittenExpression, null);
    ...
    public Mock<T> SetupProperty<TProperty>(Expression<Func<TAnalog, TProperty>> expression, TProperty? initialValue = default)
    {
        ...
        return mock.SetupProperty(rewrittenExpression, initialValue);
    }
    ...
    public ISetupSequentialResult<TResult> SetupSequence<TResult>(Expression<Func<TAnalog, TResult>> expression)
    {
        ...
        var setup = Mock.SetupSequence(mock, rewrittenExpression);
    ...
    public void VerifyGet<TProperty>(Expression<Func<TAnalog, TProperty>> expression, Times? times = null, string? failMessage = null)
    {
        ...
        Mock.VerifyGet(mock, rewrittenExpression, times ?? Times.AtLeastOnce(), failMessage);
    ...
    
    {
        ...
        {
            if (node.Object is ParameterExpression left && left.Type == duckType)
            {
                var targetParameter = Expression.Parameter(targetType, left.Name);
        ...
        {
            if (node.Expression is ParameterExpression left && left.Type == duckType)
            {
                var targetParameter = Expression.Parameter(targetType, left.Name);
